<template>
    <div class="bg-dark-blue p-4 shrink-0 flex flex-col gap-2">
        ...
        <div class="group text-white text-[0.9rem] font-normal inline-block cursor-pointer" @click="toggleMenu" v-on-click-outside="hideMenuIfVisible">
            <img src="~/assets/icons/no_image.svg" alt="" class="inline object-scale-down w-8 mr-1"/>
...
<script setup lang="ts">
    ...
    const switchLocaleBasedOnActive = () => {
        ...
        const logout = async () => {
            ...
            useCSRF().clearToken();
            useUserSidebarStore().clearMenuItems();
            navigateTo('/login');
        }
